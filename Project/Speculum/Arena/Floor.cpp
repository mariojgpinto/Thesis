 ...
 *
 */
void Floor::set_plane(ntk::Plane *plane){
	if(!plane) return;

	this->_plane.set(plane->a,plane->b,plane->c,plane->d);
}
	...
	for(int j = 0 ; j < depth.rows ; j++){
		for(int i = 0 ; i < depth.cols ; i++){
			...
			if(depth(p)){
				...
				dist = this->_plane.distanceToPlane(pf);
...
bool Floor::load_from_file(tinyxml2::XMLDocument *doc, tinyxml2::XMLElement *root){
	...
	if(this->_flags[Floor::AREA]){
		...
		bool result = ToolBoxXML::cv_load_image_xml(elem_area,_XML_FLOOR_ELEM_AREA_MASK,_area_mask);
		if(!result) 
			return false;
	...
	if(this->_flags[Floor::FLOOR_MASK]){
		...
		bool result = ToolBoxXML::cv_load_image_xml(elem_mask,_XML_FLOOR_ELEM_MASK_MASK,this->_floor_mask);
		if(!result) 
			return false;
